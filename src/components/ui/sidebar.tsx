		...
		return (
			...
			>
				<Sheet.Content
					...
					classNames={{
						content:
							"bg-sidebar w-(--sidebar-width-mobile) text-sidebar-fg [&>button]:hidden",
					}}
...
const SidebarSection = ({
	...
	return (
		<div
			...
			className={cn(
				"flex flex-col in-data-[sidebar-intent=fleet]:px-0 px-2 gap-y-0.5",
				className,
			)}
			{...props}
		>
	...
	return (
		<Disclosure
			...
			className={composeTailwindRenderProps(
				className,
				"px-2 in-data-[sidebar-intent=fleet]:px-0",
			)}
			{...props}
		/>
	);
...
const SidebarSeparator = ({
	...
	return (
		<Separator
			ref={ref}
			className={cn(
				"mx-auto my-2 w-[calc(var(--sidebar-width)-theme(spacing.6))]",
				className,
			)}
			{...props}
		/>
	...
	if (!collapsed) {
		return (
			<Text
				...
				slot="label"
				className={cn(
					"flex flex-1 w-full overflow-hidden whitespace-nowrap",
					className,
				)}
				{...props}
			>
